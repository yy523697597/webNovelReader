$(function() {
	...
	function eventHandler() {
		...
		$('.foot-nav-night').on('click', function() {
			// 用于取消夜间模式后，重新激活当前的主题
			// 如果当前主题不是夜间模式的主题，在点击夜间模式按钮的时候为当前的主题添加一个old-active类	
			//如果初始化主题就是夜间主题，就为棕色主题添加一个old-active类
			if ($('.bg-color-active').attr('id') !== 'bg_color_blue') {
				$('.bg-color-active').addClass('old-active');
			} else {
				$('#bg_color_brown').addClass('old-active');
			}
			...
			if ($(this).html() === '夜间') {
				...
				// 为拥有old-active的主题添加active，并且取消同辈的old-active和active
		...
		$('#nav_pannel_bg').on('click', 'li', function(event) {
			...
			switch ($target.id) {
				case 'bg_color_pink':
					$fiction_container.css('backgroundColor', '#F7EEE5');
					Util.storageSetter('current_bg_color', '#F7EEE5');
					Util.storageSetter('current_font_color', '#000');
					break;

				case 'bg_color_brown':
					$fiction_container.css('backgroundColor', '#E9DFC7');
					Util.storageSetter('current_bg_color', '#E9DFC7');
					Util.storageSetter('current_font_color', '#000');
					break;

				case 'bg_color_tan':
					$fiction_container.css('backgroundColor', '#A4A4A4');
					Util.storageSetter('current_bg_color', '#A4A4A4');
					Util.storageSetter('current_font_color', '#000');
					break;

				case 'bg_color_green':
					$fiction_container.css('backgroundColor', '#CDEFCE');
